...
function TrailSystemList({ trailSystems }) {
  ...
  const [formData, setFormData] = useState({ sort: "", search: "", trailsPerPage: 8 });
  ...
  let items = [];
  for (let number = 1; number <= Math.ceil(trailCount / formData.trailsPerPage); number++) {
    items.push(
  ...
  const handlePagination = (e) => {
    ...
    if (e.target.textContent === "‹Previous" || e.target.textContent === "‹") {
      ...
      setActive(active - 1);
    } else if (e.target.textContent === "«First" || e.target.textContent === "«") {
      start = 0;
      setActive(1);
    } else if (e.target.textContent === "›Next" || e.target.textContent === "›") {
      start = active * formData.trailsPerPage;
      setActive(active + 1);
    } else if (e.target.textContent === "»Last" || e.target.textContent === "»") {
      start = (Math.ceil(trailSystems.length / formData.trailsPerPage) - 1) * formData.trailsPerPage;
  ...
  const handleFormChange = (e) => {
    ...
    setTrailsToDisplay(
      trailSystems.slice((active - 1) * formData.trailsPerPage, (active - 1) * formData.trailsPerPage + formData.trailsPerPage)
    );
  };
